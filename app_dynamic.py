import datetime # 記得確認最上方有 import datetime，如果沒有請在最上面補上
...
with st.sidebar:
    ...
    selected_option = st.selectbox("切換地圖上的演算法路線：", dropdown_options)
    
    # 取出真正演算法名稱 (例如把 "🚗 SA (143.13 km)" 切割成 "SA")
    # 如果名稱有空格 (例如 "2-Opt" 或 "Greedy")，要小心處理
...
# ==========================================
# 3. 頂部 KPI 儀表板 (加入真實時鐘推算)
# ==========================================
# 設定早上 9:00 全體車隊從萬和宮出發
start_time_dt = datetime.datetime(2026, 1, 1, 9, 0, 0) 

# 把最快完賽的分鐘數 (fastest_time_val) 轉成 timedelta，並加到出發時間上
...
# ★ 升級這張卡片：把「預計完賽時間」也放進去！
col3.metric(
    "⚡ 最快完賽耗時", 
    f"{fastest_h} 小時 {fastest_m} 分", 
    f"預計 {finish_time_str} 全員抵達" # 這裡把原本的文字替換成實際下班時間！
)
...
has_history = "history" in algo_data and len(algo_data["history"]) > 0
...
# 如果這個演算法有歷史紀錄，產生時間軸
if has_history:
    st.info("✨ 這個演算法支援動態過程回放！請拖拉下方的時間軸觀察路線演化：")
    history_list = algo_data["history"]
    max_step = len(history_list) - 1
    
    # 建立滑桿
    step = st.slider("演算法尋優進度 (時間軸)", min_value=0, max_value=max_step, value=max_step)
    
    # 根據滑桿位置抽出當時的數據
    current_frame = history_list[step]
    r1 = current_frame["route1"]
    r2 = current_frame["route2"]
    current_cost = current_frame["cost"]
    iter_num = current_frame["iteration"]
    
    # 顯示當時的狀態
    st.write(f"**目前顯示：** 迭代次數 / 步驟 `{iter_num}` ｜ 當下總距離：**{current_cost} km**")
else:
    st.write("*(此演算法目前僅顯示最終收斂結果)*")
...
with col_a: st.plotly_chart(fig2, use_container_width=True)
with col_b: st.plotly_chart(fig3, use_container_width=True)